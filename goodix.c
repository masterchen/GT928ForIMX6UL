...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
	...
	set_bit(EV_SYN, ts->input_dev->evbit);
        ...
        input_set_abs_params(ts->input_dev, ABS_X, 0, 1280, 0, 0);
        input_set_abs_params(ts->input_dev, ABS_Y, 0, 800, 0, 0);
        input_set_abs_params(ts->input_dev, ABS_PRESSURE, 0, 1023, 0, 0);
#endif
...
//	GTP_INFO("GTP device node: %s", *np = dev->of_node);
//	GTP_INFO("GTP gtp_rst_gpio: %s", gtp_rst_gpio);
//	GTP_INFO("GTP gtp_int_gpio: %s", gtp_int_gpio);		
}
